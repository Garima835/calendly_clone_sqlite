...
import { FiCalendar, FiClock, FiUser, FiMapPin, FiArrowRight } from 'react-icons/fi';
...
export default function UserProfile() {
    ...
    const loadProfile = async () => {
        try {
            const res = await API.get(`/event-types/user/${username}`);
            setData(res.data);
        } catch (error) {
            setData(null);
        } finally {
    ...
    if (!data) {
        return (
            <div className="page-loading">
                <FiUser size={48} />
                <h2 style={{ marginTop: '16px' }}>User not found</h2>
        ...
        <div className="user-profile-page">
            ...
            {data.eventTypes.length === 0 ? (
                <div className="empty-state">
                    ...
                    <p>This user hasn't set up any bookable events yet</p>
                </div>
            ) : (
                    ...
                    <div className="up-events-grid">
                        {data.eventTypes.map(et => (
                            <Link
                                ...
                                style={{ '--et-color': et.color || '#0D9488' }}
                            >
